...
public class Ex08GradeManagerFile {
	...
	// add student
	static void addStudent(String student[][]) throws Exception {
	...
	static boolean deleteStudent(String student[][]) throws Exception {
	...
	// update Student
	static void updateStudent(String student[][]) throws Exception {
	...
	static void fileSave(String student[][]) throws Exception {
		...
		String fileDir = "d:/student/studentData.txt";
		FileWriter fw = new FileWriter(fileDir); // 파일 내용을 써줄 수 있도록 객체를 생성한다.
	...
	// 파일 불러오기
	static void fileLoad(String student[][]) throws Exception {
		String fileDir = "d:/student/studentData.txt";
		...
		// 공백이 아닐때까지 반복문 실행하기
		while ((str = br.readLine()) != null) {
			// 파일에서 문장을 읽어올 때 split을 활용해 ',' 토큰 단위로 끊어서 문장 잘라주기
			String splitArr[] = str.split(",");
			for (int j = 0; j < student[0].length; j++) {
				student[num][j] = splitArr[j];
			}
			num++;
			// 잘라낸 각 문장을 배열에 넣어주기
		}
